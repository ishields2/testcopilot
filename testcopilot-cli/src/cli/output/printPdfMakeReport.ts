...
function buildTable(issues: any[], flags: PdfReportFlags) {
    // Table columns depend on explain/detailedResults flags
    const columns = [
    ...
    // Add explanation/fix columns if explain is true
    if (flags.explain) columns.push({ text: 'Explanation', style: 'tableHeader' });
    if (flags.explain) columns.push({ text: 'Suggested Fix', style: 'tableHeader' });
    const body = [columns];
    for (const issue of issues) {
        ...
        if (flags.explain) row.push({ text: issue.plainExplanation || '', style: 'tableCell', noWrap: false });
        if (flags.explain) row.push({ text: issue.fix || '', style: 'tableCell', noWrap: false });
        body.push(row);
    }
...
export async function printPdfMakeReport(codebaseReport: CodebaseOutput, outputPath: string = 'testcopilot-report.pdf', flags: PdfReportFlags) {
    ...
    for (const fileResult of codebaseReport.fileResults) {
        ...
        // Only show file summary if detailedResults is true
        if (flags.detailedResults && fileResult.plainSummary) {
